...
public class KafkaConfig {
    ...
    /**
     * Failure policy (same as every consumer): poison records ({@link
     * BatchListenerFailedException}) retry briefly then dead-letter individually; infrastructure
     * failures retry the batch indefinitely with a capped backoff. Here "infrastructure" also
     * covers Elasticsearch being unreachable or returning a 5xx — indexing is idempotent (id =
     * cacheId), so replaying the batch never duplicates a document.
     */
    @Bean
    ...
    /** Elasticsearch connectivity / 5xx problems that retrying can heal. */
