...
public class KafkaConfig {
    ...
    /**
     * Failure policy (same as the other consumers):
     * <ul>
     ...
     *       poison: the batch is retried indefinitely with a capped backoff. Dispatch creation
     *       is idempotent per (incident, unit type), so retries are safe.</li>
     * </ul>
     */
    @Bean
    ...
    /** Unit lifecycle updates — consumed by response-time/SLA tracking (roadmap phase 2). */
    @Bean
    public NewTopic unitStatusTopicDefinition() {
        return TopicBuilder.name(unitStatusTopic)
                .partitions(3)
                .replicas(3)
    ...
    /** Avro producer for {@link UnitStatusEvent}s, keyed by incident id for per-incident ordering. */
    @Bean
    public KafkaTemplate<String, UnitStatusEvent> unitStatusKafkaTemplate() {
     ...
     * listener re-serialize through Avro like any normal event.
     */
    @Bean
