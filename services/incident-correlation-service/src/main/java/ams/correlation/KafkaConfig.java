...
public class KafkaConfig {
    ...
    /**
     * Failure policy (same as the other consumers): poison records ({@link
     * BatchListenerFailedException}) retry briefly then dead-letter individually;
     * infrastructure failures retry the batch indefinitely with a capped backoff —
     * correlation is idempotent per report id, so retries are safe.
     */
    @Bean
    ...
    /** Correlated-incident lifecycle events (OPENED / UPDATED / CLOSED). */
    @Bean
    public NewTopic incidentTopicDefinition() {
    ...
    /**
     * The dead-letter topic must be at least as durable as the business topics. RF 3 / min
     * ISR 2 matches them. (Existing topics are left untouched; the k8s dev overlay pre-creates
     * it at RF 1.)
     */
    @Bean
    ...
    /**
     * Producer for incident events AND the dead-letter recoverer. Serializers are selected by
     * payload type: raw bytes stay byte-for-byte (deser-failure DLT records), everything else
     * (IncidentEvent, replayed models) goes through Avro.
     */
    @Bean
